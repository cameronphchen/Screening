...
screening_function_handle = {@rand,@lasso_screening_ST, @lasso_screening_DT};
...
if strmatch(filenames{1}, options.exp_to_run)
  fprintf('noscreening\n')
...
if strmatch(options.exp_to_run{i}, options.exp_to_run) 
...
% run ST
%{
if strmatch('ST', options.exp_to_run) 
  fprintf('ST\n')
...
% run DT
%{
if strmatch('DT', options.exp_to_run) 
  fprintf('DT\n')
...
hold on
for i = 2:length(options.exp_to_run)
  plot(lambda, solve_w_screening_time(:,i)./solve_w_screening_time(:,i),'*b-') 
end
